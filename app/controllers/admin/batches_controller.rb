...
private
  ...
  def show_action?(name, resource = resource_class)
    if name == :create_request
      ...
      # check if current student has already enrolled or sent enrollment request in this batch or not 
      student = current_user.becomes(Student)
      return false if student.enrollments.pluck(:batch_id).include?(resource.id)
      return true
    else
      return true
    end
